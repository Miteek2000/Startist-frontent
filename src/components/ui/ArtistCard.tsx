...
export function ArtistCard({ artist }: ArtistCardProps) {
  const tecnicas = artist.tecnicas_completadas ?? [];

  return (
    <div
      className="max-w-2xl bg-white rounded-3xl shadow-sm hover:shadow-md transition-shadow p-6 flex flex-col"
    ...
    >
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="text-sm" style={{ color: '#815629' }}>
            {artist.descripcion}
          </p>
        </div>
      </div>
      ...
      <div className="w-full">
        ...
        >
          <span className="text-white text-ls font-semibold w-full text-left mb-2">
            TÉCNICAS
          </span>
          ...
          {tecnicas.length > 0 ? (
            ...
            <span className="text-white text-sm px-3 py-1" style={{ opacity: 0.7 }}>
              Este artista aún no cuenta con técnicas registradas
            </span>
          )}
        </div>
      </div>
